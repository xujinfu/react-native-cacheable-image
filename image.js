    ...
    _processSource(source) {
        if (source !== null
            ...
            if (Array.isArray(this.props.useQueryParamsInCacheKey)) {
                ...
                Object.keys(url.query).forEach(function (k) {
                    if (url.query[k]) {
                        cacheable = cacheable.concat(url.query[k]);
                    } else {
                        cacheable = cacheable.concat(k);
                    }
                })
            }

            // ignore extension
            let type = url.pathname.replace(/.*\.(.*)/, '$1');
            if(type && type.indexOf("/")>=0){
                type="gif";
            }
            let cacheKey = SHA1(cacheable) + '.' + (type.length < url.pathname.length ? type : 'gif');
